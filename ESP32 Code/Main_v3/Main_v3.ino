...
#include <RTClib.h> // Added RTC library
...
RTC_DS3231 rtc;       // Will use address 0x68
File dataFile;
...
void setup() {
  ...
  // Write CSV Header to the brand-new file (Updated to use DateTime instead of ms)
...
void loop() {
  ...
  if (currentMicros - lastSecTime >= oneSecondInterval) {
    ...
    if (dataFile) {
      ...
      // Force write to physical storage immediately
      dataFile.flush(); 
    }
